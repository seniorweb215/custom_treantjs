...
;( function() {
    ...
    var Tree = function (jsonConfig, treeId ) {
        ...
        this.reset = function( jsonConfig, treeId ) {
            ...
            this._R.canvas.style.overflow = "visible";
            return this;
        };
    ...
    Tree.prototype = {
        ...
        firstWalk: function( node, level ) {
            ...
            if ( (node.childrenCount() === 0 || level == this.CONFIG.maxDepth) ) {
                ...
                if ( leftSibling ) {
                    ...
                    if(node.modifier < 0) {
                        node.prelim -= node.modifier;
                        node.modifier = 0;
                    }
                
                    if(node.spouseId) {
                        this.apportion( node, level, true );
                    } else {
            ...
            if(node.spouseCount() > 0) {
                for ( var i = 0, n = node.spouseCount(); i < n; i++ ) {
                    this.firstWalk(node.spouseAt(i), level);
                    // if(leftSibling) {
                    //     // this.apportion(node.spouseAt(i), level, true);
                    // }
                }
            }

            if(node.siblingCount() > 0) {
                for(var i = 0, n = node.siblingCount(); i < n; i++) {
                    this.firstWalk(node.siblingAt(i), level);
                }
            }
         ...
         */
        apportion: function (node, level, spouse = false) {
            ...
            while( firstChild && firstChildLeftNeighbor && compareDepth <= depthToStop ) {
                ...
                for ( var i = 0; i < compareDepth; i++ ) {
                    if(leftAncestor.parent() === undefined && spouse === false) {
                        break;
                    }
                    
                    if(leftAncestor.spouseNode()) {
                        for(;;) {
                            leftAncestor = leftAncestor.spouseNode();
                            if(leftAncestor.parent()) {
                                break;
                            }
                        }
                    ...
                    if(rightAncestor.spouseNode()) {
                        rightAncestor = rightAncestor.spouseNode().parent();
                    } else {
            ...
            if ( level <= this.CONFIG.maxDepth ) {
                ...
                if (node.pseudo) { // pseudo nodes need to be properly aligned, otherwise position is not correct in some examples
                    ...
                    if(node.parentId == 0) {
                        ...
                        var tempId = id - 1;
                        if(orient == "NORTH" || orient == "SOUTH") {
                            node.X = (this.nodeDB.get(1).X + this.nodeDB.get(tempId).X) / 2;
                        } else if(orient == "EAST" || orient == "WEST") {
                            node.Y = (this.nodeDB.get(1).Y + this.nodeDB.get(tempId).Y) / 2;
                        }
        ...
        positionNodes: function() {
            ...
            for ( i = 0, len = this.nodeDB.db.length; i < len; i++ ) {
                ...
                if (node.id !== 0 && node.parent() && !(node.parent().id === 0 && this.CONFIG.hideRootNode)) {
                ...
                if(node.parentId === 0 && node.pseudo === true && !this.CONFIG.hideRootNode) {
                    // var len = node.id;
                    for(var j = 1; j < node.id; j++) {
                        var parentNode = this.nodeDB.get(j);
                        this.setConnectionToSibling(parentNode, node, hidePoint);
                    }
                    
                }

                if(node.spouseNode()) {
                    this.setConnectionToSpouse(node, hidePoint); // skip the root node
                }
         ...
         * @returns {Tree}
         */
        setConnectionToSpouse: function( treeNode, hidePoint ) {
            var stacked = treeNode.stackParentId,
                connLine,
                spouse = treeNode.spouseNode(),
                pathString = hidePoint?
                    this.getPointPathString(hidePoint):
                    this.getPathString(spouse, treeNode, stacked, true);
                // pathString = this.getPathString(spouse, treeNode, stacked, true);
            if ( this.connectionStore[treeNode.id] ) {
                // connector already exists, update the connector geometry
                connLine = this.connectionStore[treeNode.id];
                this.animatePath( connLine, pathString );
                ...
                // don't show connector arrows por pseudo nodes
                if ( treeNode.pseudo ) {
                    delete spouse.connStyle.style['arrow-end'];
                }
                if ( spouse.pseudo ) {
                    delete spouse.connStyle.style['arrow-start'];
                }

                connLine.attr( spouse.connStyle.style );
         ...
         * @returns {Tree}
         */
        setConnectionToSibling: function( parentNode, treeNode, hidePoint ) {
            var stacked = treeNode.stackParentId,
                connLine,
                parent = parentNode,
                pathString = this.getPathString(parent, treeNode, stacked);

            if ( this.connectionStore[treeNode.id][parent.id] ) {
                // connector already exists, update the connector geometry
                connLine = this.connectionStore[treeNode.id][parent.id];
                this.animatePath( connLine, pathString );
            }
            else {
                connLine = this._R.path( pathString );
                this.connectionStore[treeNode.id][parent.id] = connLine;
                // don't show connector arrows por pseudo nodes
                if ( treeNode.pseudo ) {
                    delete parent.connStyle.style['arrow-end'];
                }
         ...
         * @returns {string}
         */
        getPathString: function( from_node, to_node, stacked, spouse = false ) {
            var startPoint = from_node.connectorPoint( true, spouse ),
                endPoint = to_node.connectorPoint( false, spouse ),
        ...
        setNeighbors: function( node, level ) {
            ...
            if(node.id == 0 && node.sibling) {
                node.rightNeighborId = node.sibling[0];
            }

            if(node.spouseId) {
                node.leftNeighborId = node.spouseId;
    ...
    NodeDB.prototype = {
        ...
        reset: function( nodeStructure, tree ) {
             ...
             */
            function iterateChildren( node, parentId, spouse = false) {
                var newNode;
                if(spouse == false && !(parentId == 0 && node.pseudo && tree.CONFIG.hideRootNode)) {
                    newNode = self.createNode( node, parentId, tree, null );
                    newNode.sibling = [];
                } else {
                    newNode = self.get(parentId);
                }
                newNode.spouse = [];

                if(parentId == -1 && node.sibling && !tree.CONFIG.hideRootNode) {
                    
                    for(var i = 0, len = node.sibling.length; i < len; i++) {
                        var connStyle = UTIL.cloneObj( newNode.connStyle );
                        siblingNode = self.createNode( node.sibling[i], -1, tree, null );
                        siblingNode.connStyle = connStyle;
                        newNode.sibling.push(siblingNode.id);
                    }

                }

                if ( node.spouse ) {
                    for( var i = 0, len = node.spouse.length; i < len; i++) {
                        var connStyle = UTIL.cloneObj( newNode.connStyle );
                        newNode = self.createNode( node.spouse[i], newNode.id, tree, null, true );
                        newNode.connStyle = connStyle;
                        newNode.spouse = [];
                        iterateChildren(node.spouse[i], newNode.id, true);
                    }
                }
         ...
         * @returns {TreeNode}
         */
        createNode: function( nodeStructure, parentId, tree, stackParentId, spouse = false) {
            var node = new TreeNode( nodeStructure, this.db.length, parentId, tree, stackParentId, spouse );
                ...
                if ( parentId >= 0 ) {
                    ...
                    if ( nodeStructure.position ) {
                        if ( nodeStructure.position === 'left' ) {
                            parent.spouse.push( node.id );
                        }
                        else if ( nodeStructure.position === 'right' ) {
                            parent.spouse.splice( 0, 0, node.id );
                        }
                        else if ( nodeStructure.position === 'center' ) {
                            parent.spouse.splice( Math.floor( parent.spouse.length / 2 ), 0, node.id );
                        }
                        else {
                            // edge case when there's only 1 child
                            var position = parseInt( nodeStructure.position );
                            if ( parent.spouse.length === 1 && position > 0 ) {
                                parent.spouse.splice( 0, 0, node.id );
                            }
                            else {
                                parent.spouse.splice(
                                    Math.max( position, parent.spouse.length - 1 ),
                                    0, node.id
                                );
                            }
                        }
     ...
     * @constructor
     */
    var TreeNode = function( nodeStructure, id, parentId, tree, stackParentId, spouse = false ) {
        this.reset( nodeStructure, id, parentId, tree, stackParentId, spouse );
    };
    ...
    TreeNode.prototype = {
         ...
         * @returns {TreeNode}
         */
        reset: function( nodeStructure, id, parentId, tree, stackParentId, spouse = false ) {
            this.id = id;

            if(spouse == true) {
                this.spouseId = parentId;
            } else {
         ...
         * @returns {number}
         */
        spouseCount: function () {
            return ( !this.spouse)? 0: this.spouse.length ;
        },
        ...
        siblingCount: function () {
            return ( ( this.collapsed || !this.sibling || this.hideRootNode)? 0: this.sibling.length );
        },
         ...
         * @returns {TreeNode}
         */
        spouseAt: function( index ) {
            return this.dbGet( this.spouse[index] );
        },
         ...
         * @returns {TreeNode}
         */
        siblingAt: function( index ) {
            return this.dbGet( this.sibling[index] );
        },
        ...
        spouseNode: function() {
            return this.lookupNode( this.spouseId );
        },
        ...
        leftSibling: function () {
            ...
            if ( leftNeighbor) {
                ...
                var leftSibling = leftNeighbor;

                if(this.spouseId === leftNeighbor.id) {
                    return leftNeighbor;
                }
                for(;;) {
                    if(!leftSibling) {
                    ...
                    if(leftSibling.parentId == 0) {
                        if(leftSibling.parentId == parent) {
                            return leftNeighbor;
                        }
                    }

                    if(leftSibling.parentId && leftSibling.parentId == parent) {
                        return leftNeighbor;
                    }
                    
                    if(leftSibling.collapsed) {
                        leftSibling = leftSibling.leftNeighbor();
                    } else {
                        leftSibling = leftSibling.spouseNode();
                    }
                }
            }
        },
        ...
        rightSibling: function () {
            ...
            if ( rightNeighbor && (rightNeighbor.parentId === this.parentId || this.id === rightNeighbor.spouseId) ) {
                return rightNeighbor;
            }

            if ( rightNeighbor) {
                var parent = rightNeighbor.parentId;
                var rightSibling = this;
                ...
                for(;;) {
                    ...
                    if(rightSibling.parentId == 0) {
                        if(rightSibling.parentId == parent) {
                            return rightNeighbor;
                        }
                    }

                    if(rightSibling.parentId && rightSibling.parentId == parent) {
                        return rightNeighbor;
                    }
                    
                    rightSibling = rightSibling.spouseNode();
                }
            }
        },
        ...
        collapsedParent: function() {
            ...
            var spouseNode = this.spouseNode();
            ...
            if ( spouseNode && spouseNode.collapsed ) {
                return parent;
            }
            if(parent) {
                return parent.collapsedParent();
            }
            return spouseNode.collapsedParent();
        },
        ...
        connectorPoint: function(startPoint, spouse = false) {
    ...
    var JSONconfig = {
        make: function( configArray ) {
            ...
            while(i--) {
                ...
                // top parent from data
                if (!node.hasOwnProperty('parent') && ! node.hasOwnProperty('container') && !node.hasOwnProperty('spouse')) {
        ...
        findChildren: function(nodes) {
            ...
            while(parents.length) {
                ...
                for(;i<len;i++) {
                    ...
                    if(node.spouse && node.spouse._json_id === parentId) {
                        node._json_id = this.getID();
